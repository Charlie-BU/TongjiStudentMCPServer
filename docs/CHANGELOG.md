...
- 2026-09-23 19:54（Asia/Shanghai）

### Base Commit

- `9a9748f95d81cfa5b2a413ee213351c655dee25b`（沿用历史记录格式，取 `HEAD~1`，仅作基线元数据）。

### Compare Scope

- `working_tree_only`：全部当前未提交改动，相对 `HEAD`（`85d2e798dbdc0c044ef167ddbbc8f3a5d9fd5425`）比较。此前已提交的校园工具扩展、服务凭据协议及联系方式结果校验不作为本次新增实现。

### 背景与改动目标

原有工具名称将通用校园能力归在 student、card 或 research 下，无法直接反映适用人群。本次统一使用 user、teacher、student、bachelor、postgraduate 五类命名空间，并在工具描述中说明适用范围，同时移除一卡通消费汇总工具。

### 改动概览

- `campusTool` 增加必填的 `audience` 元数据，统一生成“使用范围”描述；42 个校园工具全部显式声明人群。该字段由开发者配置，不作为模型输入参数。
- 重命名 15 个工具并同步移动目录、注册入口、测试和文档，不保留旧名称别名。重命名对照见 [同济 API 文档](TONGJI_API.md#使用人群与命名)。
- 本科成绩、绩点学分汇总和竞赛奖励迁至 `tongji.bachelor.*`；年度账单、借阅、校园统计、学期日历、门禁、校园卡及科研查询统一迁至 `tongji.user.*`。
- 删除 `tongji.card.spending_summary` 的注册、工具实现、响应 Schema 和手写集成方法，保留 CAM 自动生成接口。
- 更新工具目录导出说明，移除已不注册的会议创建提示；同步 README、工具目录、迁移说明和测试规范。
- 新增五类命名空间、使用范围描述及工具数量一致性测试；现有契约测试改用新名称。

### 关键链路解析（含上下游）

- 工具发现：注册时将 `audience` 映射为中文使用范围，工具描述保留原有业务说明与写操作提示。当前 42 个校园工具按 user 15、teacher 2、student 13、bachelor 3、postgraduate 9 分组。
- 工具调用：调用方使用新名称查找工具，重命名后的工具继续调用原有集成方法。现有请求身份限定、业务字段映射和错误处理沿用基线实现；使用人群标记仅提供选择指引，不新增服务端角色鉴权。
- Agent 配套：同工作区 Agent allowlist 已同步。实际 MCP tools/list 与 Agent 名单逐项比对，均为 59 个工具，无缺项或多余项。

### 改动结果与业务影响

- 当前注册 59 个工具：42 个校园工具、6 个公开课程/历史评价工具、11 个瑞幸工具。
- 工具名称和描述可区分通用用户、本科生、研究生、全体学生与教师能力。
- 本次包含不兼容的工具重命名与删除；旧名称调用将不可用，Agent、技能引用和其他 MCP 客户端需配套更新。消费汇总能力不再通过 MCP 暴露。

### 风险与待办

- 审阅期间重跑全量离线测试，242 项全部通过；源码与测试 TypeScript 类型检查、暂存区及工作区差异检查通过。测试通过本地 Node/tsx 与 TypeScript 执行。
- 已验证 59 个实际注册工具与同工作区 Agent allowlist 一致；不代表已部署版本完成同步。
- 未调用真实校园接口或执行真实联系方式修改，也未验证模型对人群描述的选择效果。人群说明不代替上游权限校验。
- 本次仅新增 changelog，沿用刚完成的审阅验证结果，未重复运行测试。

### 建议 Commit Message（git-cz）

- `refactor(mcp): organize campus tools by audience and remove spending summary`

## CHANGELOG - 2026-09-23 18:42 - 扩展校园工具并切换服务凭据与用户身份协议

### 撰写时间

- 2026-09-23 18:42（Asia/Shanghai）

### Base Commit

- `1265ab1f5a9d549e8754bf869f752c4744485d5d`（沿用历史记录格式，取 `HEAD~1`，仅作基线元数据）。

### Compare Scope

- `working_tree_only`：全部当前未提交改动，相对 `HEAD`（`d78a921232e1ceb46a05b5b8f42eceec78aef689`）比较，包含暂存区与本轮审阅修复。Agent 配套实现不作为本仓新增功能。

### 背景与改动目标

校园数据访问改用服务 Token，并由 Agent 显式传递已验证的当前用户 ID。在更新 CAM 接口契约的基础上扩展校园查询能力，将校园工具注册、认证、错误处理和响应校验收敛到公共执行层。本轮同时修复修改联系方式仅检查外层状态码、可能将内层失败误报成功的问题。

### 改动概览

- 更新 CAM 生成客户端及类型；手写集成拆分为 `adapter.ts`、`methods.ts` 和统一导出入口，集中注入服务凭据和用户 ID、处理上游业务错误。
- HTTP 入口要求 `X-Tongji-Access-Token` 与 `X-Tongji-User-Id` 成对提供，并通过 basic-info 核验固定服务账号。缺失、重复或不合法的身份头按匿名处理；服务凭据校验失败返回 401。
- 新增 26 个校园工具，覆盖研究生成绩与培养计划、考试安排、本科绩点、一卡通余额与汇总、科研、资助、教职工信息、联系方式、邮箱及辅导员。保留的 17 个校园工具迁入 `campusTool` 公共执行层，合计 43 个校园工具；连同 6 个课程/评价工具和 11 个瑞幸工具，当前注册 60 个工具。
- 移除原有 `tongji.course.calendar_list` 和 `tongji.user.basic_info` 工具注册及实现；人员基础信息集成仍用于服务凭据校验。
- 校园工具使用严格输入 Schema，拒绝模型传入身份或凭据；新增响应 Schema，保留既有工具字段映射、空结果、参数回显和分页契约，并裁剪或脱敏响应数据。
- 成绩及课表支持数字学期编号；校门、图书馆通行对接新 CAM 参数与游标，兼容旧时间/方向参数。住宿、竞赛、荣誉、奖学金和助学金保留分页元数据。
- 学生详情和瑞幸工具直接使用请求上下文中的用户 ID，不再通过服务 Token 推断当前用户。公开 YourTJ 与本地评价查询保持匿名可用。
- 新增 `docs/TONGJI_API.md` 和工具目录导出脚本，更新 README、工具目录、测试说明及 YourTJ 文档。

### 关键链路解析（含上下游）

- 身份：Agent 验证用户 Token 后发送用户 ID 与服务 Token；MCP HTTP 入口校验服务凭据，适配器覆盖业务参数中的身份字段，CAM 按各接口定义发送单用户查询参数。MCP 不申请或刷新服务 Token。
- 查询：公共执行层检查身份和业务参数，调用手写集成，校验与转换响应，输出 MCP 文本及 structuredContent；异常统一返回脱敏的工具错误。
- 写操作：修改联系方式要求明确操作意图，且至少提供手机号或邮箱。本轮增加内层结果判定，只有 `data.code=A00000` 且 `effectRows` 为正安全整数时报告 `ok`；内层失败、零更新、缺失或异常字段均返回结果尚未确认、先核实且不要自动重试的提示。该判断不添加任何自动重试。

### 改动结果与业务影响

- 校园能力扩展到 43 个工具，查询范围由 Agent 提供的可信用户 ID 限定；服务账号身份不会被当作用户身份用于瑞幸凭据查询。
- 联系方式修改不会再仅凭外层成功码向用户承诺修改成功。
- 这是配套部署变更：Agent 必须发送新的身份头并同步工具 allowlist；旧的仅用户 Token 调用方式不再满足个人工具认证要求。已移除工具的调用方需调整。

### 风险与待办

- 全量离线测试 242 项全部通过，新增用例覆盖内层业务失败、零更新、异常条数、空结果、字段缺失及成功响应脱敏，并验证每次调用仅发起一次上游请求。
- 测试 TypeScript 类型检查、生产 TypeScript 编译构建和工作区差异检查通过。由于 pnpm 版本切换此前受注册源访问限制，本轮直接使用本地 Node/tsx 和 TypeScript 执行对应检查，未通过 pnpm 包装命令运行。
- 未访问真实校园或瑞幸接口，未实际修改联系方式。联系方式内层成功码及更新条数约定、各新增接口实际响应与服务账号权限仍需受控联调确认；零更新保守处理为结果未确认。
- 服务身份检查依赖上游 basic-info，服务 Token 申请、缓存与刷新仍由 Agent 负责。

### 建议 Commit Message（git-cz）

- `feat(mcp): expand campus tools and validate scoped service operations`
...
- 上游依赖：`src/index.ts` 继续通过 `loadServerConfig` 读取 `PORT`；Agent 仍通过 `X-Tongji-Access-Token` 将短期凭据传给 `/mcp`。本次没有改变入口参数、token 传递方式或 CAM 生成客户端。
...
- 上游依赖：`src/index.ts` 读取 `PORT` 后创建 Node HTTP server；`StreamableHTTPServerTransport` 负责 MCP 协议解析，`McpServer` 负责承载注册后的工具目录。
...
- 已执行 `pnpm typecheck`、`pnpm build` 和 `git diff --check HEAD`。沙箱内监听本地 `3000` 端口被权限策略拒绝；在获批环境中启动命令不再出现 SDK 子路径加载错误，但仍需要在真实本地或部署环境完成 `/health` 与 MCP initialize 冒烟验证。
