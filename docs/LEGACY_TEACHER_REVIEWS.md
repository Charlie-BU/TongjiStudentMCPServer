...
Node.js 要求 22.13+，使用 `node:sqlite`；开发与测试使用 tsx 时禁用新版 Node 的原生类型剥离以避免旧版 tsx 的模块加载冲突。生产启动仍为 `node dist/index.js`。数据库路径相对模块定位，部署必须保留同级 `seed/`，不依赖启动目录。
...
一次性导入脚本、清单和报告已移除，运行时读取常驻库 `data/mcp.sqlite`。数据库不存在或评价表为空时，自动从随代码发布的 `seed/teacher-reviews.seed.sqlite` 导入全部 2704 条评价，保留原始 ID、教师姓名及正文；已有非空评价表保持不变。部署须同时携带种子库与构建产物，不能用种子库覆盖运行库。执行 `pnpm check` 验证数据库结构、检索、HTTP 路由与 MCP 工具。
