...
export const registerLegacyTeacherReviewsTool = (server: McpServer): void => {
    server.registerTool(LEGACY_TEACHER_REVIEWS_TOOL_NAME, {
        title: "检索老师历史评价",
        description: "按老师姓名或姓名片段模糊检索评价，返回所有匹配老师的全部评价。保留课程和原始学期；属于历史学生主观评价，不一定代表当前情况。",
        inputSchema: { teacher: legacyTeacherNameSchema.describe("老师姓名或姓名片段（如“陈”），按连续子串匹配，自动去除首尾空白。") },
