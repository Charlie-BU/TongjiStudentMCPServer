...
export const BOOK_LEND_RECORD_SCHEMA = z.object({
    ...
    deptCode: z.string().nullable().describe("单位代码"),
    deptName: z.string().nullable().describe("单位名称"),
    docTypeCode: z.string().nullable().describe("文献类型代码"),
    docTypeName: z.string().nullable().describe("文献类型"),
    isbn: z.string().nullable().describe("ISBN编号"),
    langCode: z.string().nullable().describe("书籍语种代码"),
    langName: z.string().nullable().describe("书籍语种"),
    lendDate: z.string().nullable().describe("借出日期"),
    locationCode: z.string().nullable().describe("馆藏地代码"),
    locationName: z.string().nullable().describe("名称"),
    name: z.string().nullable().describe("姓名 读者姓名，注意该字段未做脱敏处理，不可在公开输出中直接引用。"),
    propNo: z.string().nullable().describe("财产号"),
    pubYear: z.string().nullable().describe("出版年"),
    ...
    userId: z.string().nullable().describe("学工号 学工号，注意该字段未做脱敏处理，不可在公开输出中直接引用。"),
});
